...
unsafe fn thread_func() {
    ...
    STRING_ADDR = Some((base_ga + 0x00130F00) as *mut u8);
    ...
    module_manager.enable(MhyContext::<Http>::new(Some((base_ga + 0x08402F50) as *mut u8)));
    ...
    module_manager.enable(MhyContext::<SdkUtil>::new(Some((base_ga + 0x0736FDA0) as *mut u8)));
    module_manager.enable(MhyContext::<SdkUtil>::new(Some((base_ga + 0x079569B0) as *mut u8)));
    ...
    module_manager.enable(MhyContext::<DisableCensorship>::new(Some((base_ga + 0x011E0300) as *mut u8)));
    module_manager.enable(MhyContext::<DisableCensorship>::new(Some((base_ga + 0x011E0650) as *mut u8)));

    // 48 89 5C 24 ? 57 48 83 EC 30 80 3D ? ? ? ? ? 41 8B F8 0F 29 74 24 ? 48 8B D9 0F 28 F1 75 2E 80 79 33 00 74 18 F3 0F 10 0D
    module_manager.enable(MhyContext::<DisableCensorship>::new(Some((base_ga + 0x011E05C0) as *mut u8)));

    // 48 89 5C 24 ? 57 48 83 EC 70 80 3D ? ? ? ? ? 48 8B D9 0F 29 74 24 ? 0F 28 F2 0F 29 7C 24
    module_manager.enable(MhyContext::<DisableCensorship>::new(Some((base_ga + 0x011E0390) as *mut u8)));
